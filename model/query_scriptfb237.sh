...
python create_queries.py --index_only --gen_train_num 273710 --gen_test_num 8000 \
 ...
 --dataset 'fb15k-237' --sg_loc 'results/48/datasets' \
 --path '../dataset/FB15k-237-multimodal/' \
...
for i in 1 2 3 4
do
  python create_queries.py --gen_train_num 165689 \
  --gen_train --gen_id $i \
  --dataset 'fb15k-237' --sg_loc 'results/48/datasets' \
  --path '../dataset/FB15k-237-multimodal/' \
  --query_folder 'query_generation' --max_ans_num 100 --per_mm 0.3
...
do
  ...
  python create_queries.py --gen_train_num 30968 \
  --gen_train --gen_id $i \
  --dataset 'fb15k-237' --sg_loc 'results/48/datasets' \
  --path '../dataset/FB15k-237-multimodal/' \
  --query_folder 'query_generation' --max_ans_num 100 --per_mm 0.3
